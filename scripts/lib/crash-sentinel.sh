#!/usr/bin/env bash
#
# Crash sentinel for the app under test, run after every upgrade and restore test phase.
#
...
app_crash_lines() {
    ...
    # CRs stripped first: a dump taken through a pty ends lines in CRLF, and "ANR in <pkg>" at the
    # end of a line would then fail the `( |$)` anchor.
    tr -d '\r' < "$logcat_file" | grep -E \
